...
from SaveData import saveIterationResults
...
#def getPrediction_fromRaw(model, data):
#    new_model = Model(inputs=model.input[0], outputs=model.output[1])
#    return new_model.predict(data)

# outputs embedding for given raw data
def getEmbedding(model, graph_name, data):
    new_model = Model(inputs=model.get_layer('Input_Target'+graph_name).input, outputs=model.get_layer('Embedding_Target'+graph_name).get_output_at(0))
    if data.shape[1]>1:
...
#    if data.shape[1]>1:
#        data = data.reshape((data.shape[0],1,data.shape[1]))
    return model.get_layer('Branch_Prediction').predict(data)#.squeeze()
...
def train(load_path, models, A, Afeatures, Alabels, Atrain, Atest, B, Bfeatures, Blabels, transmode, topology_similarity, n_walks, walk_length, window_size, p, q, n_negative, learning_rate, beta, nepochs, early_stopping, batch_size, save_path, suffix, output_file):
    ...
    print("\n=======================================\nDATA IS LOADING")
    from GraphProcessing import getNodeSimilarity
    Atarget, Acontext, Asimilarity, Anegative = getNodeSimilarity(load_path, 'A', A, mode=topology_similarity, n_walks=n_walks, walk_length=walk_length, window_size=window_size, p=p, q=q, n_negative=n_negative, features=Afeatures)
    if B is None:
        Btarget, Bcontext, Bsimilarity, Bnegative = None, None, None, None
    else:
        Btarget, Bcontext, Bsimilarity, Bnegative = getNodeSimilarity(load_path, 'B', B, mode=topology_similarity, n_walks=n_walks, walk_length=walk_length, window_size=window_size, p=p, q=q, n_negative=n_negative, features=Bfeatures)
    print("\n=======================================\nDATA IS LOADED")
    ...
    if transmode not in ['1graph', 'noP']:
        ...
        print("\nEMBEDDING BRANCH A INITIALIZATION:", file=output_file)
        EmbeddingABranch.fit_generator(Data, epochs=1)
        # adjust embedding of graph A by learning class labels
        print("\nPREDICTION BRANCH INITIALIZATION:", file=output_file)
        PredictionModel.fit_generator(DG.PredictionNodesGenerator(batch_size, Afeatures[Atrain,:], Alabels[Atrain,:]), epochs=1)
        # compute training and testA accuracies
        epochs.append(0)
        Aembedding = getEmbedding(EmbeddingModel, 'A', Afeatures)
        ...
        print("\nP INITIALIZATION:")
        if transmode == 'trueP':
            P = np.loadtxt(load_path+'ABCorrespondence.txt')
        elif transmode == 'trueP_DS':
            P = np.loadtxt(load_path+'ABCorrespondence_DS.txt')
        else:
            from updateP import updateP
            P = updateP(A, B, np.zeros(A.shape), None, 'opt')
        # update A,B embeddings w.r.t. transfer loss
        W_TargetA = EmbeddingABranch.get_layer('Embedding_TargetA').get_weights()[0]
        W_TargetB = np.zeros(W_TargetA.shape)
        W_TargetA, W_TargetB = updateWeigths(W_TargetA, W_TargetB, P, lr=0.1)
        ...
        try:
            ...
            W_ContextB = np.zeros(W_ContextA.shape)
            W_ContextA, W_ContextB = updateWeigths(W_ContextA, W_ContextB, P, lr=0.1)
        ...
        EmbeddingBBranch.fit_generator(Data, epochs=1)
        ...
        print("\n---------------------------------------\nAfter model initialization\nTrain accuracy = %.4f (graph A)\t:\tTest accuracy = %.4f (graph A)\t:\tTest accuracy = %.4f (graph B)" % (acc_train[-1], acc_testA[-1], acc_testB[-1]), file=output_file)
        acc_txt.write("{0:d}\t{1:.4f}\t{2:.4f}\t{3:.4f}\n".format(epochs[-1], acc_train[-1], acc_testA[-1], acc_testB[-1]))
    else:
        P = None
    ...
    for epoch in range(nepochs):
        ...
        if transmode not in ['1graph', 'noP']:
            if transmode == 'trueP':
                ...
                if beta:
                    ...
                    # TODO: choose good scale factor and function
                    scale = scalefactor^(scalefunc(epoch-nepochs/2))
                else:
                    scale = 1.0
                D = scale*computeDistanceBetweenGraphs(getEmbedding(EmbeddingModel,'A',Afeatures), getEmbedding(EmbeddingModel,'B',Bfeatures))
                P = updateP(A, B, D, P, transmode) # optimize permutation matrix P
                print("P is updated with '%s' method" % transmode)
        else:
        ...
        EmbeddingModel.fit_generator(Data, epochs=1)
        ...
        if (Alabels.shape[1]>1 and acc_curr>acc_best) or (Alabels.shape[1]==1 and acc_curr<acc_best):
            print("\nEPOCH #{0:3d} :: Train accuracy has improved from the previous best value at epoch #{1:3d}: {2:.4f} -> {3:.4f}".format(epoch, epoch_best, acc_best, acc_curr), file=output_file)
            patience = 0
            epoch_best = epoch
            acc_best = acc_curr
            ...
            print("\nEPOCH #{0:3d} :: Train accuracy ({1:.4f}) has not improved from the previous best value at epoch #{2:3d} ({3:.4f})".format(epoch, acc_curr, epoch_best, acc_best), file=output_file)
            patience += 1
        ...
        if epoch==0 or (epoch+1)%step==0 or epoch==nepochs-1:
            ...
            if B is None:
                print("\n---------------------------------------\nTrain accuracy = {0:.4f} (graph A)\t:\tTest accuracy = {1:.4f} (graph A)\n---------------------------------------\n".format(acc_train[-1], acc_testA[-1]), file=output_file)
                acc_txt.write("%4d\t%.4f\t%.4f\n" % (epochs[-1], acc_train[-1], acc_testA[-1]))
            else:
                Bembedding = getEmbedding(EmbeddingModel, 'B', Bfeatures)
                Blabels_output = getModelOutputs_fromEmbedding(PredictionModel, Bembedding)
                acc_testB.append(computeAccuracy(Blabels, Blabels_output))
                print("\n---------------------------------------\nTrain accuracy = {0:.4f} (graph A)\t:\tTest accuracy = {1:.4f} (graph A)\t:\tTest accuracy = {2:.4f} (graph B)\n---------------------------------------\n".format(acc_train[-1], acc_testA[-1], acc_testB[-1]), file=output_file)
                acc_txt.write("%4d\t%.4f\t%.4f\t%.4f\n" % (epochs[-1], acc_train[-1], acc_testA[-1], acc_testB[-1]))
    ...
    # save best models' outputs and results
    epochs.append(epoch_best+1)
    acc_train.append(acc_best)
    
    # compute, save and output best results
    print("\n---------------------------------------\nRestoring best model state from epoch #%4d:" % epochs[-1], file=output_file)
    Aembedding = getEmbedding(EmbeddingModel, 'A', Afeatures) # get graph A node embeddings
    Alabels_output = getModelOutputs_fromEmbedding(PredictionModel, Aembedding)
    ...
    if B is None:
        ...
        if Alabels.shape[1]>1:
            print("Train accuracy = %.4f (graph A)\t:\tTest accuracy = %.4f (graph A)" % (acc_train[-1], acc_testA[-1]), file=output_file)
            acc_txt.write("-"*24 + "\n  %4d\t%.4f\t%.4f\n" % (epochs[-1], acc_train[-1], acc_testA[-1]))
        else:
            rsquared = computeRSquared(Alabels[Atest,:], Alabels_output[Atest,:], np.mean(Alabels[Atrain,:]))
            best_results['acc_rsquaredA'] = rsquared
            print("Train accuracy = %.4f (graph A)\t:\tTest accuracy = %.4f (graph A)\t:\tR-squared = %.4f (graph A)" % (acc_train[-1], acc_testA[-1], rsquared), file=output_file)
            acc_txt.write("-"*32 + "\n  %4d\t%.4f\t%.4f\t%.4f\n" % (epochs[-1], acc_train[-1], acc_testA[-1], rsquared))
    else:
        Bembedding = getEmbedding(EmbeddingModel, 'B', Bfeatures) # get graph B node embeddings
        Blabels_output = getModelOutputs_fromEmbedding(PredictionModel, Bembedding)
        ...
        if Alabels.shape[1]>1:
            print("Train accuracy = %.4f (graph A)\t:\tTest accuracy = %.4f (graph A)\t:\tTest accuracy = %.4f (graph B)" % (acc_train[-1], acc_testA[-1], acc_testB[-1]), file=output_file)
            acc_txt.write("-"*32 + "\n  %4d\t%.4f\t%.4f\t%.4f\n" % (epochs[-1], acc_train[-1], acc_testA[-1], acc_testB[-1]))
        else:
            ...
            print("Train accuracy = %.4f (graph A)\t:\tTest accuracy = %.4f (graph A)\t:\tR-squared = %.4f (graph A)\t:\tTest accuracy = %.4f (graph B)\t:\tR-squared = %.4f (graph B)" % (acc_train[-1], acc_testA[-1], rsquaredA, acc_testB[-1], rsquaredB), file=output_file)
            acc_txt.write("-"*48 + "\n  %4d\t%.4f\t%.4f\t%.4f\t%.4f\t%.4f\n" % (epochs[-1], acc_train[-1], acc_testA[-1], rsquaredA, acc_testB[-1], rsquaredB))
    
    acc_txt.close()
    
    # save iteration results
    saveIterationResults(save_path, suffix, Aembedding, Alabels_output, Alabels_predict, Bembedding, Blabels_output, Blabels_predict, P, acc_results)
    
    return best_results


# train embedding branch solely
def train_embedding(load_path, models, A, Afeatures, topology_similarity, n_walks, walk_length, window_size, p, q, n_negative, nepochs, batch_size):
    
    EmbeddingModel = models['EmbeddingModel']
    
    import time
    print("Run Keras GTL embedding")
    starttime = time.clock()
    Data = DG.EmbeddingNodesGenerator(load_path, batch_size, A, topology_similarity=topology_similarity, n_walks=n_walks, walk_length=walk_length, window_size=window_size, p=p, q=q, n_negative=n_negative, Afeatures=Afeatures)
    ...
    XaT=[]
    XaC=[]
    AembeddingT = getEmbedding(EmbeddingModel, 'A', Afeatures, layer_name='Target')
    AembeddingC = getEmbedding(EmbeddingModel, 'A', Afeatures, layer_name='Context')
    for n in range(len(A)):
        XaT.append(AembeddingT[n,:]/np.linalg.norm(AembeddingT[n,:]))
        XaC.append(AembeddingC[n,:]/np.linalg.norm(AembeddingC[n,:]))
    XaT=np.array(XaT)
    XaC=np.array(XaC)
    simMatT=np.zeros(A.shape)
    simMatC=np.zeros(A.shape)
    for i in range(len(A)):
        for j in range(len(A)):
            simMatT[i,j]=np.dot(XaT[i,:],XaT[j,:])
            simMatC[i,j]=np.dot(XaC[i,:],XaC[j,:])
    return XaT, simMatT, XaC, simMatC
