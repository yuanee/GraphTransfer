...
def getRandomWalks(load_path, flag, adj_mat, p, q, n_walks, walk_length):
    
#    filename = load_path + 'Graph' + flag + 'Walks.pkl'
#    try:
#        with open(filename,'rb') as handle:
#            walks = pickle.load(handle)
#    except:
...
#            print 'Save random walks to ' + load_path + filename
#            pickle.dump(walks, handle, protocol=pickle.HIGHEST_PROTOCOL)
    return walks


def getRWSimilarity(load_path, flag, adj_mat, n_walks, walk_length, window_size, p, q, n_negative):
    """
    ...
    """
#    if n_negative > 0:
    walks = getRandomWalks(load_path, flag, adj_mat, p, q, n_walks, walk_length)
    target, context, similarity, negative_samples = skip_train(walks, window_size, n_negative)
#    else:
#        negative_samples = None
#        try:
#            filename = 'Graph' + flag + 'RWSimilarity_' + str(window_size) + '.pkl'
#            with open(load_path + filename, 'rb') as handle:
#                rw = pickle.load(handle)
#            print 'Loaded random walk similarity from file'
#            node_pairs = rw['node_pairs']
#            similarity = rw['similarity']
#        except:
#            print 'Compute random walk similarity'
...
#            negative_samples = None
#            for walk in walks:
#                couples, labels = skipgrams(np.array(walk)+1, len(adj_mat), window_size=window_size, negative_samples=1., shuffle=True, categorical=False)
#                node_pairs.extend(list(np.array(couples)-1))
#                similarity.extend(map(float,labels))
#            node_pairs = np.array(node_pairs)
#            similarity = np.array(similarity)
#        with open(load_path + filename, 'wb') as handle:
#            print 'Save random walks similarity to ' + load_path + filename
#            pickle.dump(dict(zip(['node_pairs','similarity'],[node_pairs,similarity])), handle, protocol=pickle.HIGHEST_PROTOCOL)
