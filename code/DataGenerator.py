...
#    def __init__(self, load_path, batch_size, A, B=None, topology_similarity=None, n_walks=10, walk_length=10, window_size=5, p=.25, q=4, n_negative=0, P=None, Afeatures=None, Bfeatures=None):
...
#        self.Atarget, self.Acontext, self.Asimilarity, self.Anegative = getNodeSimilarity(load_path, 'A', A, mode=self.topology_similarity, n_walks=n_walks, walk_length=walk_length, window_size=window_size, p=p, q=q, n_negative=self.n_negative, features=Afeatures)
...
#            self.Btarget, self.Bcontext, self.Bsimilarity, self.Bnegative = getNodeSimilarity(load_path, 'B', B, mode=self.topology_similarity, n_walks=n_walks, walk_length=walk_length, window_size=window_size, p=p, q=q, n_negative=self.n_negative, features=Bfeatures)
...
class SimpleDataGenerator(Sequence):
    ...
    def __getitem__(self, idx):
        ...
        source = self.source[idx*self.batch_size : (idx+1)*self.batch_size,:]
        negative = self.negative[idx*self.batch_size : (idx+1)*self.batch_size,:]
        similarity = self.similarity[idx*self.batch_size : (idx+1)*self.batch_size,:,:]
        return [target, source, negative], [similarity]
